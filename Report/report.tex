...
Twitters API och systemets användning av det lyder under följande begränsningar:
	...
	\begin{itemize}
     	\item Meddelanden som är äldre än en vecka kan inte hämtas in (Twitter, 2012). Twitters sök-API kan bara användas för att hitta meddelanden som är {\it ungefär en vecka} gamla ({\it omkring 6-9 dagar} i en annan del av dokumentationen). Detta är inget problem så länge basstationen är aktiverad, men så fort den stängs av och sätts på igen kommer den enbart att ha tillgång till meddelanden från den senaste veckan. Om det senaste meddelandet är äldre kommer den felaktigt att rapportera till skylten att inga meddelanden finns tillgängliga. Om skylten sedan stängs av (exempelvis för att laddas) kommer den vid uppstart inte att ha tillgång till det meddelande som den visade upp före avstängning.
...
Genom kontinuerlig kommunikation via Google Groups, mail och veckomöten har samtliga gruppmedlemmar hållts involverade och uppdaterade. Kommunikationen bedöms ha fungerat utmärkt, med endast mindre missförstånd, såsom någon dubbelbokning av grupprum för möte. Arbetet har flutit på mycket bra, och inga konflikter inom gruppen eller större problem har uppkommit. De problem som faktiskt stötts på under projektet har varit rent konstruktionsmässiga eller elektronikmässiga, och har kunnat lösas. Vid de möten som hållts varje vecka har gruppen disktuerat projektets status och vad som har behövs göras. Beslut har tagits gemensamt i gruppen. Vid några tillfällen, såsom val av microcontrollerplattform och val av display, har det funnits olika alternativ som diskuterats inom gruppen. Vid dessa tillfällen har alternativen jämförts och beslut tagits med hela gruppens samtycke.\\

Kommunikationen med handledaren har fungerat bra, denne har medverkat vid ungefär varannat gruppmöte. Vidare har handledaren kontaktats via mail då det behövts, samt kunnat följa projektets gång via loggboken. Handledaren har bidragit med värdefullt stöd och bra feedback under hårdvaruinköp samt under rapportskrivningen.

\subsubsection{Arbetsfördelning}
I efterhand kan sägas att upplägget med att fördela konstruktionsmoment till par av två gruppmedlemmar har varit lyckat. På detta sätt har medlemmarna inom ett arbetspar kunnat stödja varandra på ett bra sätt, samtidigt som man kunnat arbeta på tre parallella konstruktionsmoment samtidigt inom gruppen, genom tre grupper av två medlemmar vardera. En detalj som skulle kunna justeras är att en av arbetsgrupperna arbetat mycket på distans, vilket i sig inte är ett problem, men ibland har det varit önskvärt att ha en gruppmedlem från varje arbetspar på plats i arbetsrummet. Samtliga gruppmedlemmar har bidragit till projektet, och utfört de uppgifter man kommit överens om utan problem. Samarbetet i gruppen har fungerat mycket bra, utan några egentliga problem.
	
\subsubsection{Hårdvarubeställningar}
Inköpen av hårdvara under projektet har fungerat smidigt via handledare och ansvariga på instutitionen. Det har inte varit problem på Chalmers sida att
få den hårdvara gruppen velat ha. Ett misstag från gruppens sida som gjordes tidigt i projektet var dock att vänta för länge med beställningen av display. Då gruppen väl beslutat att använda E-Paper, visade det sig vara för lång leveranstid på sådana displayer. Istället fick man använda sig av en bistabil LCD från Kent, vilket i efterhand dock visade sig vara ett fullt jämförbart alternativ till E-Paper. Anledningen till att gruppen sköt upp beställningen av display var att tillräcklig information saknades, och att E-Paper ansågs som något av en risk på grund av dess höga komplexitet jämfört med en normal LCD.\\
...
\bibitem{rfc2277}
    RFC2277,
    \url{http://tools.ietf.org/html/rfc2277}
